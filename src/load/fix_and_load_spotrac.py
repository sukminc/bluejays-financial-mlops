...
def load_clean_data():
    ...
    try:
        ...
        # 3. [CRITICAL] Nuclear Option: Force Drop Old Table
        # We manually drop the table to ensure the schema reset
        print("   💥 Nuke old table (DDL)...")
        with engine.begin() as conn:
            conn.execute(text(
                "DROP TABLE IF EXISTS stg_spotrac_bluejays_salary_raw"
            ))

        # 4. Load New Data (Will Create Table from Scratch)
